...
object Anagrams extends AnagramsInterface:
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def recur(freqs : Occurrences): List[Sentence] = {
      if( freqs.isEmpty) List(Nil)
      else 
        for {
          combin <- combinations(freqs)
          word <- dictionaryByOccurrences(combin)
          rest <- recur(subtract(freqs, combin))
        } yield word :: rest
    }
